...
MODEL_PATH = MODELS_DIR / "amazon_sentiment_model.joblib"
...
with st.sidebar:
    ...
    if metrics:
        ...
        st.write(f"N-gram range: `{metrics.get('config', {}).get('ngram_range', 'n/a')}`")

review = st.text_area(
    "Review text",
    height=220,
    placeholder="Write a review here, for example: This product is amazing and works exactly as expected.",
    key="review_input",
)
